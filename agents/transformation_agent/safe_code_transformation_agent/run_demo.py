...
def run_planner_adapter_demo(adapter: PlannerAdapter, output_dir: Path, test_data_dir: Path) -> None:
    ...
    rdp_plan = payload["rdp_sample"]
    java_source = payload["example_source_code"]["java"]

    request_payload = adapter.build_request_from_rdp(
        request_id="planner_demo_001",
        language="java",
        source_code=java_source,
        planner_output=rdp_plan,
        correlation_id=rdp_plan.get("metadata", {}).get("correlation_id"),
    )
    write_json(output_dir / "planner_adapter_request.json", request_payload)
    print("[DEMO] planner adapter request generated")
