...
THIS IS ONE OF TWO IMPLEMENTATIONS of the same fourteen functions; the other is
db/supabase_repository.py. Callers import neither: they import
db/workflow_repository.py, which picks one at call time. The two must keep the
same signatures and return the same shapes — a dict per row, JSON columns left
as strings for parse_json_field to decode.
